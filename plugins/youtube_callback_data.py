    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "Ses", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("Dosya",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "Video", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("Dosya",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("İndiriliyor...", callback_data="down")]]))
    filepath = os.path.join(userdir, filext)
    # await q.edit_message_reply_markup([[InlineKeyboardButton("İşleniyor..")]])
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("Yükleniyor...", callback_data="down")]]))
